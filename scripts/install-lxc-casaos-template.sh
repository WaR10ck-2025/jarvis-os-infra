...
systemctl start casaos-app-management 2>/dev/null || true
sleep 3

# Idempotent registrieren via CLI
for i in $(seq 1 6); do
  casaos-cli app-management list app-stores 2>/dev/null | grep -q "casaos-store.zip" && break
